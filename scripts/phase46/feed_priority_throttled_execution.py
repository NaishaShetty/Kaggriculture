"""
Phase 46 Part C: scripts/phase46/feed_priority_execution.py (Part B's
FEED-tier-0.5 reprioritization, itself composed on top of Phase 36's
validated BUY_ANIMAL cash pacer) PLUS one more layer: a capacity-aware
BUY_ANIMAL throttle. Everything below is identical to
scripts/phase46/feed_priority_execution.py except the BUY_ANIMAL section,
which now additionally caps owned+in-flight animal count at
ANIMAL_TO_HAND_THROTTLE_RATIO * current_hands before allowing a further
purchase -- see the ANIMAL_TO_HAND_THROTTLE_RATIO constant below for the
rationale (Part B alone left the escape reduction modest and mixed across
seeds; the goal here is a smaller, reliably-fed animal count instead of
continuing to chase the full target rung).

--- Part B's own docstring, describing the (unmodified) FEED reprioritization
this file composes with ---

Phase 46 Part B: a further layer on top of Phase 36's already-validated
BUY_ANIMAL cash pacer (scripts/phase36/paced_execution.py, reused/composed
here -- this file starts as an exact copy of that module, per this phase's
explicit instruction to wrap/compose it rather than reimplement it, and
changes EXACTLY ONE THING: the FEED task's priority tier, from 1 (tied with
WATER) to 0 (tied with HARVEST/HARVEST_ANIMAL, strictly ahead of WATER).
Every other line -- BUY_ANIMAL pacing, targets, HIRE/BUY_LAND/BUY_SEED,
the rest of the task-priority scheme, the greedy nearest-worker matching
itself -- is untouched.

WHY THIS CHANGE, GROUNDED DIRECTLY IN PART A'S OWN MEASUREMENT
(scripts/phase46/capacity_probe.py, results/phase46/phase46_capacity_probe_summary.json):
at Submission I's real operating scale (11 hands, up to 13 owned animals,
50+ crop tiles), FEED and WATER share priority tier 1 and compete for the
SAME greedy nearest-worker assignment. Instrumenting every turn across two
full 30-day trajectories (isolated seed 700000, vs. Submission G seed
701002) found FEED's shortfall is DOMINATED by losing the tier-1
nearest-worker race to WATER (or other FEED entries) while workers WERE
still available (62.9%/65.9% of FEED-relevant turns), not by outright
worker exhaustion (27.2%/24.1% of FEED-relevant turns, i.e. tier 0 already
claimed every worker before tier 1 was even reached). Since the DOMINANT
failure mode is a same-tier priority race Reprioritizing FEED to tier 0
(strictly ahead of WATER, matching HARVEST/HARVEST_ANIMAL) directly
addresses the dominant mechanism found, without requiring any new hands or
capacity -- it reallocates EXISTING worker-turns rather than manufacturing
more of them (the approach Phase 30/32 already found doesn't pay off:
dedicating extra hands to a new task type costs more in Fibonacci hire
premiums than it returns).

RISK BEING WATCHED (per this phase's own explicit instruction, and the
Phase 36 self-defeating-feedback-loop precedent): promoting FEED ahead of
WATER could simply move the shortage onto WATER/crop-tile throughput
instead of fixing it. This is checked directly in
scripts/phase46/feed_priority_screen.py via idle-fraction and land/tile
utilization comparisons, not assumed away.

--- Original Phase 36 docstring below, describing the (unmodified) BUY_ANIMAL
pacer this file composes with ---
...
# PHASE 46 PART C: a further, additional layer on top of Part B's FEED
# tier-0.5 reprioritization. Part B's 4-seed screen showed only a modest,
# mixed reduction in escapes (mean 8.25->7.50 across 4 seeds, individual
# seeds moved in both directions) -- reprioritizing FEED helps some but does
# not reliably close the gap to the target rung. Per this phase's brief,
# Part C throttles further BUY_ANIMAL purchases once owned-animal count
# gets large relative to available hands (the simpler proxy the brief
# explicitly allows, rather than a live escape-rate estimator, since the
# engine gives no direct "animal escaped this turn" signal in `obs` --
# escapes can only be reconstructed after the fact from ledger data, not
# used causally inside the same agent that would need to react to them in
# real time). GOAL: a smaller, reliably-fed animal count rather than a
# larger chronically-escaping one, per the brief's own framing.
ANIMAL_TO_HAND_THROTTLE_RATIO = 0.7  # cap owned+in-flight animals at 0.7x current hand count


def make_feed_priority_throttled_execution_agent(target_fn, seed_buffer_cap=6):
    ...
    def agent(obs):
        ...
        # PHASE 46 PART C ADDITION: also capacity-throttled -- no further
        # purchase is made once owned+in-flight animal count already meets
        # or exceeds ANIMAL_TO_HAND_THROTTLE_RATIO * current hand count.
        # This is a hard cap on SIMULTANEOUS animal count, not a one-time
        # target change -- it re-arms every turn, so a purchase becomes
        # possible again once escapes bring the count back below the cap
        # (mirroring how the existing system already naturally "refills"
        # after an escape, just at a lower ceiling).
        n_hands_now = len(me.get("hands", []))
        animal_cap = ANIMAL_TO_HAND_THROTTLE_RATIO * max(n_hands_now, 1)
        need_animal_by_type = {}
        ...
        for atype, cnt in need_animal_by_type.items():
            ...
            if still_needed <= 0:
                continue
            room_under_cap = max(0, int(animal_cap) - owned_animal_count)
            if room_under_cap <= 0:
                continue
            cost = ANIMALS[atype]["cost"]
            r = animal_reserve_now()
            affordable_units = int((money - r) // cost) if money > r else 0
            buy_n = min(still_needed, MAX_ANIMAL_PURCHASES_PER_TURN, affordable_units, room_under_cap)
            if buy_n > 0:
        ...
        for t in structure_tiles:
            ...
            if tt is None:
                ...
                if tt.get("yield_units", 0) > 0:
                    ...
                    # PHASE 46: FEED moved from tier 1 (tied with WATER) to
                    # its OWN tier 0.5 -- strictly ahead of WATER (tier 1)
                    # but NOT merged into HARVEST/HARVEST_ANIMAL's tier 0.
                    # An earlier version of this file tried a literal tier 0
                    # merge (FEED sharing HARVEST's own priority number) and
                    # found a decisive, severe regression on the 4-seed
                    # isolated screen (-89.5% mean money, idle_fraction
                    # roughly doubling 0.08->0.17): merging FEED into
                    # HARVEST's tier makes FEED (and its WHEAT fetch
                    # entries, since req_min_priority elevates with it)
                    # compete directly with time-sensitive crop harvests for
                    # the SAME nearest-worker pass, which starves HARVEST
                    # instead of fixing FEED -- exactly the kind of
                    # self-defeating substitution this phase was told to
                    # watch for. Tier 0.5 gives FEED strict precedence over
                    # WATER (this phase's actual target, per the brief's
                    # "or otherwise gives it precedence over WATER
                    # specifically" alternative) without ever contesting
                    # HARVEST's own tier.
