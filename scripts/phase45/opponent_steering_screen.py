"""
Phase 45 Part B: 4-seed screen, head-to-head, Submission I shipped (baseline)
vs. the opponent-aware crop-steering variant, against all three required
opponents (Submission G, Submission C, the Phase 43 Jonaid archetype). Run
first, cheaply, before committing to the mandatory full 15-seed validation --
same two-stage discipline as Phase 42's own screen.
"""
import json
import os
import sys
...
from scripts.phase45.opponent_aware_portfolio import make_opponent_aware_portfolio_agent  # noqa: E402
...
CANDIDATES = {
    ...
    "opponent_aware": make_opponent_aware_portfolio_agent,
}
...
def main(seeds, seed_label):
    ...
    out_path = os.path.join(OUT_ROOT, f"phase45_opponent_steering_{seed_label}.json")
