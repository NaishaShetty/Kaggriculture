"""
Phase 51: a HIRE pacer, extending Phase 36/45's cash-flow-ADAPTIVE purchase
pacer pattern to HIRE -- the one purchase type BUY_LAND/BUY_ANIMAL/BUY_SEED
now all pace but HIRE itself still doesn't.

PER THIS PHASE'S OWN CONSTRAINT: `agents/phase21/execution.py` and
`scripts/phase46/feed_priority_execution.py` are NOT edited directly (both
stay exactly as Submission K ships them, still importable and
byte-identical). This module is a phase51-local wrapper that reproduces the
one remaining monolithic block (the single `agent(obs)` closure
`make_feed_priority_execution_agent` builds, which has no pluggable seam for
HIRE specifically) with ONLY the HIRE block changed. Every other line
(BUY_LAND, BUY_SEED, BUY_ANIMAL's Phase 36 pacing, the FEED tier-0.5 reorder
from Phase 46, the BUY_PRODUCT feed logic, and the entire task-scheduling
section) is copied verbatim from `scripts/phase46/feed_priority_execution.py`
-- this composes Submission K's own shipped execution layer (Phase 46's
FEED-priority fix), it does not reimplement it. Phase 48's capped-animal
wrapper is layered on top of THIS module by the caller, exactly the same way
`scripts/phase48/capped_animal_portfolio_agent.py` layers it on top of
`scripts/phase46/feed_priority_execution.py` (see
`hire_paced_capped_animal_portfolio_agent.py` in this directory).

READ FIRST (per this phase's own instruction), confirmed directly from
source (`agents/phase21/execution.py` lines 191-203, byte-identical in
`scripts/phase46/feed_priority_execution.py`):
    ...
    for _ in range(affordable):
        market.append(["HIRE"])
    money -= cum
HIRE affordability-caps the day's cumulative hire cost against the FULL
`money` balance -- no reserve held back at all, unlike BUY_LAND (which
already holds `land_purchase_reserve`), BUY_ANIMAL (Phase 36's own reserve,
built on top of `land_purchase_reserve`), and BUY_SEED (Phase 45's pacer,
gated on the identical `land_purchase_reserve`). HIRE is the last of the
four purchase types with a genuinely zero reserve.

THE NEW GATE, reusing the SAME land_purchase_reserve/posture-aware concept
BUY_LAND/BUY_ANIMAL/BUY_SEED already use (per this phase's explicit
instruction, not a new number invented from scratch):

    HIRE_BUDGET(state) = money - land_purchase_reserve (posture-aware,
                          Phase 26's grounded $75/$150/$300 -- the identical
                          value the other three purchase types already
                          reserve against, reused a fourth time here)

The cumulative fib-cost affordability loop is otherwise IDENTICAL to the
original -- only the balance it is checked against changes, from `money` to
`money - land_purchase_reserve`.

WHY THIS MIGHT MATTER, PER THE BRIEF'S OWN FRAMING (Phase 33's original
trace, corroborated directly by Phase 48 Part B's retest through the newer
FEED-priority execution layer): the 4th-quadrant dedicated-hands TOMATO/GOOSE
design has twice been diagnosed to fail for the SAME reason -- the
daily-recurring Fibonacci HIRE cost for the extra dedicated hands delays the
agent's own core land ramp (BUY_LAND competes with HIRE for the same cash),
pushing the SE quadrant purchase to day 24-26, far too late for TOMATO/GOOSE
to produce any revenue before the 30-day game ends. This is a HIRE
cash-timing problem, not the worker-turn-scheduling problem Phase 46 already
fixed (confirmed directly by Phase 48: HIRE's code was byte-identical
between the old and new execution layers, untouched by the FEED/WATER tier
reorder). A HIRE pacer that reserves cash for BUY_LAND (rather than letting
HIRE freely spend down to `land_purchase_reserve`'s own floor and then some)
is the first fix that actually targets HIRE's own affordability check --
never tried before this phase.

PER PHASE 49'S EXPLICIT LESSON (its own report, and this doc's Phase 49
update block): validating an execution-layer change independently does NOT
certify it in combination with a DIFFERENT already-validated execution-layer
change, even when they target ostensibly orthogonal resources -- Phase 49
found BUY_SEED's pacer (Phase 45) and FEED's tier reorder (Phase 46), each
independently safe, combine to a real, diffuse regression. This module is
therefore layered on top of ONLY Phase 46's FEED-priority fix (Submission
K's own shipped execution layer) -- not stacked on top of Phase 45's
BUY_SEED pacer as well, since that combination was already found unsafe and
was never shipped. The MANDATORY sanity check below (run on Submission K's
own unmodified, capped-animal portfolio, no 4th quadrant) exists precisely
to catch an unexpected interaction here too, rather than assuming this
pairing is safe just because HIRE and FEED target different resources.
"""
import os
import sys
...
FEED_BUFFER_DAYS = 2  # unchanged from Phase 36/46
WHEAT_PRICE_ESTIMATE = 30  # unchanged from Phase 36/46
MAX_ANIMAL_PURCHASES_PER_TURN = 1  # unchanged from Phase 36/46


def make_hire_paced_execution_agent(target_fn, seed_buffer_cap=6):
    """Byte-for-byte the same behavior as
    scripts/phase46/feed_priority_execution.py::make_feed_priority_execution_agent
    EXCEPT the HIRE block, which now gates the day's cumulative affordable
    hire cost against `money - land_purchase_reserve` instead of the full
    `money` balance (see module docstring)."""
    ...
    def agent(obs):
        ...
        def animal_reserve_now():
            """Unchanged from Phase 36/46 -- BUY_ANIMAL's own reserve, kept exactly."""
            return land_purchase_reserve + FEED_BUFFER_DAYS * owned_animal_count * WHEAT_PRICE_ESTIMATE

        # --- HIRE, PACED (Phase 51 -- the new piece): the day's cumulative
        # affordable hire cost is now gated against `money - land_purchase_
        # reserve` instead of the full `money` balance, reusing the SAME
        # posture-aware reserve BUY_LAND/BUY_ANIMAL/BUY_SEED already hold
        # back. The affordability loop itself (cumulative fib-cost walk) is
        # otherwise UNCHANGED from agents/phase21/execution.py /
        # scripts/phase46/feed_priority_execution.py. ---
        current_hands = len(me.get("hands", []))
        need_hire_target = max(0, n_hands - current_hands)
        ...
        # --- BUY_LAND -- UNCHANGED from Phase 46/agents/phase21/execution.py ---
        ...
        # --- BUY_SEED (per crop in play) -- unchanged from Phase 46/agents/phase21/execution.py ---
        empty_by_crop = {}
        ...
        # --- BUY_ANIMAL, paced -- UNCHANGED from Phase 36/46 ---
        need_animal_by_type = {}
        ...
        # ---------------- Task scheduling: UNCHANGED from Phase 46 (FEED at tier 0.5) ----------------
        tasks = []
        for t, c in crop_assignment.items():
            x, y = t
            tt = tiles[y][x]
        ...
        for t in structure_tiles:
            ...
            if tt is None:
                ...
                if tt.get("yield_units", 0) > 0:
                    ...
                    # Phase 46's tier 0.5 -- ahead of WATER, not merged into HARVEST's tier 0. Unchanged here.
