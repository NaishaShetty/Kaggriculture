"""
Phase 48 Part B: Phase 33 Part C's dedicated-hands 4th-quadrant TOMATO/GOOSE
design, recomposed on top of Phase 46's FEED-priority execution layer
(scripts/phase46/feed_priority_execution.py) INSTEAD of the plain execution
shape Phase 33 originally used.

WHAT PHASE 33 ORIGINALLY COMPOSED WITH (confirmed by direct read before
building this file): scripts/phase33/land_expansion_execution.py is a
freestanding reconstruction of agents/phase21/execution.py's own task loop --
its BUY_ANIMAL section is the original unthrottled, zero-reserve form (no
Phase 36 pacer), and its FEED task sits at tier 1 (tied with WATER, no Phase
46 tier-0.5 reorder). This file's base is instead an exact copy of
scripts/phase46/feed_priority_execution.py, with Phase 33's own tile-pool-
splitting logic (CORE pool = NW/NE/SW, byte-for-byte what portfolio_targets
would already plan; EXTENSION pool = SE only, TOMATO or GOOSE, dedicated
hands added on top) layered on unchanged.

WHY THIS MIGHT MATTER, PER THE BRIEF'S OWN FRAMING: Phase 33's own trace
(Section 10 of its report) found the SE quadrant wasn't bought until day 26
in the traced seed BECAUSE the daily-recurring Fibonacci hire cost for the
dedicated hands delayed the agent's own core land ramp (NE bought day 11
instead of ~day 6). That is a HIRE/cash-timing mechanism, not a worker-turn
SCHEDULING-capacity mechanism -- Phase 46's fix targets FEED/WATER tier
competition and BUY_ANIMAL pacing, neither of which touches HIRE's own
affordability-capped cumulative-fib-cost logic (both this file and Phase 33's
original leave HIRE completely unchanged, confirmed by direct read below).
So this composition is a genuine, fair retest (not a strawman -- the design
is identical to Phase 33's, just layered on the newer base), but there is no
a priori mechanical reason to expect Phase 46's fix to rescue Phase 33's
diagnosed failure mode; this is checked directly, not assumed, in the
report's Part B section.
"""
import os
import sys
...
EXTENSION_QUADRANT = "SE"
GOOSE_EXTENSION_COUNT = 6


def make_extension_feed_priority_agent(target_fn, extension_type, n_dedicated_hands=2, extension_trigger_day=12,
                                        seed_buffer_cap=6):
    ...
    def agent(obs):
        ...
        core_animal_counts = dict(base_targets["animals"])
        core_crop_tile_target = base_targets["crop_tile_target"]
        core_crop_fractions = base_targets["crop_fractions"]
