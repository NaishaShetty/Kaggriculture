"""
Phase 48 Part C validation: head-to-head, Submission J shipped (baseline,
feed_priority, uncapped animal targets) vs. the capped-animal-target
candidate (scripts/phase48/capped_animal_portfolio_agent.py, cap_total=8),
against all three required opponents (Submission G, Submission C, the Phase
43 Jonaid archetype) -- mirrors scripts/phase46/feed_priority_screen_h2h.py's
own structure exactly: cheap 4-seed dev screen first, full 15-seed set
(mandatory per this project's own standing rule) only once the screen shows
promise.
"""
import json
import os
...
CANDIDATES = {
    "baseline_j": make_feed_priority_portfolio_agent,
    "capped_animal_8": make_capped_animal_portfolio_agent,
}
...
def main(seeds, seed_label):
    ...
    out_path = os.path.join(OUT_ROOT, f"phase48_animal_cap_h2h_{seed_label}.json")
