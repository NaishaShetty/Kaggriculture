"""
Phase 33 Part B, Step 5: isolated economy + fertilized-tile-count measurement
for the strictly opportunistic (non-preempting) fertilizer configuration,
same 4 development seeds, vs. "pass" -- directly comparable to Phase 30's
own table (results/phase30/PHASE30_FERTILIZER_REPORT.md Section 4).
"""
import json
import os
import sys
...
def make_agent_with_opponent_logging(fertilize):
    ...
    execution_agent = make_fert_execution_agent(portfolio_targets, fertilize=fertilize)
...
def run_condition(label, fertilize):
    ...
    for seed in DEV_SEEDS:
        ...
        record, extracted = analyze_replay(replay, meta, "phase33_isolated_fert", f"{label}_seed{seed}")
...
def main():
    results = {}
    print("=== baseline: no fertilizer (agents/phase21/ as shipped, reproduced via the same execution shape) ===")
    results["baseline_no_fertilizer"] = run_condition("baseline_no_fertilizer", fertilize=False)

    print("=== strictly opportunistic fertilizer (tier 4 / tier 5, never preempts tier<=3) ===")
    results["opportunistic_fertilizer"] = run_condition("opportunistic_fertilizer", fertilize=True)

    os.makedirs(OUT_ROOT, exist_ok=True)
    with open(os.path.join(OUT_ROOT, "phase33_isolated_economy_fertilizer_results.json"), "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nWrote {OUT_ROOT}/phase33_isolated_economy_fertilizer_results.json")

    b = results["baseline_no_fertilizer"]["mean_final_money"]
    o = results["opportunistic_fertilizer"]["mean_final_money"]
