...
This section tracks what we actually built and shipped, submission by submission. It's project history, not game documentation — see `docs/BIG_SWING_PLAN.md` for the full phase-by-phase research log this summary is drawn from.

There are two distinct lineages. **A–E** are the original architecture: a frozen day-0 greedy-knapsack planner ("Planner v1") with a stack of sequential response layers bolted on top, each gated by a detector watching the opponent. **F–K** are a from-scratch rewrite: a single coherent macro-controller built around real replay data instead of patched-on reactions, after diagnosis showed the old architecture had a low ceiling that tuning couldn't fix.

| Submission | Phase | What it is | Outcome |
| :--- | :--- | :--- | :--- |
| **A** | 3.4 | Planner v1 + "Variant D": a crop-substitution layer that swaps MELON for STRAWBERRY once an opponent-expansion detector fires with confidence ≥ 0.5. | First real submission past the raw planner. |
| **B** | 3.5 | A + an opponent-scaling response: once the opponent's hands/animals stay ≥ 5 for 3 consecutive days, raise our own hand target. | Added a second reactive layer on top of A. |
| **C** | 3.8 | B + an animal-scaling response, piecewise on the opponent's live animal count, using the same trigger as B. | The most-tuned build of the old lineage; used as the standard benchmark opponent (`Submission C`) for every later phase. |
| **D** | 7 | A bug fix, not a new layer: Planner v1 believed it had a force-liquidation safety net (`sell_policy["horizon_aware"]`) wired in, but the code path that actually implements it was never called — traced directly to a real loss where 59 harvested units sat unsold in the shed. Fixed the import. | Real, narrow fix — only matters in the specific stuck-inventory case it targets. |
| **E** | 12 | Same story for a second dead trigger: the "F-005" liquidity guard only fired at one exact instant (`day==2, hour==0`) instead of checking across a window, so it almost never activated. Widened the check. | Provably safe (0/44 regressions) but **0/44 measured improvement** — the guard turned out to fire on a routine cash dip, not a rare crisis, so fixing its wiring didn't change outcomes. |
| **F** | 18 | The first from-scratch build: a new macro-controller (`agents/phase15/`) sized from real top-ladder replay data instead of Planner v1's greedy heuristic, reusing validated pieces (market simulator, fetch-bottleneck fix) rather than patching the old lineage again. | Genuinely different architecture, beat the old lineage on early testing. |
| **G** | 20 | F, refined and validated on a win-rate basis instead of a money-maximization basis — the actual thing Kaggle's Elo scoring rewards. | Became the standard `Submission G` benchmark opponent for every later phase. |
| **H** | 29 | A second from-scratch rewrite (`agents/phase21/`), built around three "reframes" from re-reading the engine source directly: (1) the market is one shared pool across both players, so crop choice has to account for combined two-player volume, not just solo yield; (2) win probability is the actual objective, not money; (3) "market as weapon" (deliberately crashing prices) — tested and refuted, never shipped. Diagnosed and fixed real crop-economics and per-tile endgame-liquidation-timing bugs along the way. | First submission to clear a majority win rate against the strongest prior submission: **9/15 (60%) vs. G, 14/15 (93.3%) vs. C.** |
| **I** | 37 | H + an adaptive cash-flow pacer for `BUY_ANIMAL` purchases, replacing a fixed day-indexed schedule that front-loaded capital demand into the tightest cash window. | **10/15 vs. G, 15/15 vs. C** — a real, validated improvement. |
| **J** | 47 | I + a scheduling-priority fix: `FEED` was tied with `WATER` for worker priority and lost that race 63-66% of the time, causing animals to starve and escape almost as fast as they were purchased. Gave `FEED` its own priority tier strictly ahead of `WATER` (but not merged into `HARVEST`'s tier — that literal version was tried first and was a catastrophic regression). This fix also turned out to be the shared root cause behind several previously-rejected ideas (fertilizer, dedicated-hands crop expansion) that had all been failing on the same near-zero worker-turn slack. | **14/15 vs. G (up from 10/15), 15/15 vs. C** — zero new losses introduced anywhere in validation. |
| **K** | 50 | J + a capacity-aware animal-target cap. Direct instrumentation found owned animals settle into a real steady-state equilibrium of 6-9 regardless of game length — the schedule's own target of 17 was unreachable, and every purchase past the real ceiling was money spent on an animal that would likely just escape. Capped the target at 8 (the empirically-measured ceiling) instead. | **15/15 vs. G (recovering J's one remaining loss), 15/15 vs. C, 15/15 vs. a real reconstructed 4th-quadrant opponent** — the current best, live submission. |

**A note on submission size**: A–E bundle 24-31 files (eight historical response layers, a 26KB market-model module, and a 183KB static detector artifact) because each new layer had to carry every prior layer it built on. F–K bundle 13-15 files total — one coherent controller plus whatever the current fix adds — because the rewrite doesn't drag old layers behind it. The smaller `.tar.gz` size for J/K reflects a leaner architecture, not missing content; every submission is fully self-contained and cold-process-tested by extracting it outside the repo and running a real episode against it before shipping.

**Status as of this writing**: J and K are live. Real recent-game checks (pulled directly from Kaggle's own episode replay data, not just our own synthetic validation) have shown win rates in the 60-70% range over various samples, alongside normal short-term Elo noise and the occasional losing streak. Several further ideas (fertilizer, a 4th land quadrant in three different specific forms, a `HIRE` cash-flow pacer, combining two validated fixes at once) have been tried and closed negative since K shipped — each for a distinct, precisely diagnosed reason, all recorded in `docs/BIG_SWING_PLAN.md`.
